...
do
    ...
    local menu = library.new("pc2.lua", "pc2.lua Config Folder\\")
        local tabs = {
    ...
    do
        ...
        local function update_cfgs()
            all_cfgs = listfiles("pc2.lua Config Folder\\")
            for _,cfg in next, all_cfgs do
                all_cfgs[_] = string.gsub(string.gsub(cfg, "pc2.lua Config Folder\\", ""), ".txt", "")
...
local RandomShitTalk = {
    ...
    ["Chinese"] = {
			...
			"pc2.lua pc2.lua pc2.lua 他，所以他不那r给了他东西给了他爱欠s，却拒绝参加锻炼，这让他爱得更UGT少了",
	...
	["Emoji"] = {
		...
		["Main"] = {
			"pc2.lua runs u .gg/grbkMDGMMX ",
			"newgen got 2 tapped by pc2.lua LOL ",
			"#pc2.lua better XD @reversedox",
			"pc2.lua user VV 😳😳😳",
		    "just a skill issue that you dont have pc2.lua",
		    "mad cause no pc2.lua LOL @reversedox",
		},
}
...
menu.values[5].menu.Settings["Cheat Name"].Text = "pc2.lua"
LPH_NO_VIRTUALIZE(function()
    RunService.RenderStepped:Connect(function(fps)
    Watermark.Title.Text = (menu.values[5].menu.Settings["Cheat Name"].Text:gsub("{game}", game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name):gsub("{name}", LocalPlayer.Name):gsub("{fps}", "FPS: " .. math.floor(1/fps)):gsub("{cheatname}", "pc2.lua"):gsub("{time}", os.date("%b. %d, %Y")))
